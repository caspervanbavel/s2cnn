...
if not torch.cuda.is_available():
    ...
    print("running only cpu tests")


def test_so3_rfft(b_in, b_out, device):
    x = torch.randn(
        2 * b_in, 2 * b_in, 2 * b_in, dtype=torch.float, device=device
    )  # [beta, alpha, gamma]
...
def test_inverse(f, g, b_in, b_out, device, complex):
    if complex:
        x = torch.randn(
            2 * b_in, 2 * b_in, 2 * b_in, 2, dtype=torch.float, device=device
        )  # [beta, alpha, gamma]
    else:
        x = torch.randn(
            2 * b_in, 2 * b_in, 2 * b_in, dtype=torch.float, device=device
...
def test_inverse2(f, g, b_in, b_out, device):
    x = torch.randn(
        b_in * (4 * b_in**2 - 1) // 3, 2, dtype=torch.float, device=device
    )  # [beta, alpha, gamma]
